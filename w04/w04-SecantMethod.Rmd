...
include_graphics("img/w04-secant.png")
```
...
if (knitr:::is_latex_output()) {
  knitr::asis_output('\\url{https://github.com/pengdsci/MAT325/raw/main/w04/img/w04-betterSecantAnimation.gif}')
} else {
  knitr::include_graphics("img/w04-betterSecantAnimation.gif")
}
```
...
include_graphics("img/w04-SecantFalsePosition.png")
```
...
```{r echo=FALSE, fig.align ="center",  out.width = '55%'}
if (knitr:::is_latex_output()) {
  knitr::asis_output('\\url{https://github.com/pengdsci/MAT325/raw/main/w04/img/w04-regula.gif}')
} else {
  knitr::include_graphics("img/w04-regula.gif")
}
```
...
fn = function(x){
      f = x^3+4*x^2+10
      f}                      # function input
N = 200                       # Max number of iteration
n = 0                         # initialization of iterations
a = 0                         # initialize values 
b = 2
TOL = 10^(-6)    
ERR = abs(b-a)

while (ERR > TOL){          # confirming an x value that equals 0 exists between a and b
  n = n+1                   # add iteration
  m = (b-a)/2               # calculate new x value
  if (fn(m)*fn(b)< 0){      # confirm 0 value is still within interval 
    a = m
    b = b}            
  else {
    a = a
    b = m
    }                        # assigns m as an x value depending on location of 0 
  ERR = abs(a-b)             # calculate new error value
  if (ERR < TOL){
        cat("\n\nThe algorithm converges!")                   # output for if the tolerance is reached
        cat("\n The number of integration was", n ,".")
        cat("\n The approximate root of root is r =", (a+b)/2, ".")
        cat("\n The absolute error is ", ERR, ".")
        break 
        }
  else {                                                        # output if tolerance is not yet reached
       cat ( "\nIteration :", n, " absolute value  = ", ERR,".")
      }
  if ( n == N){
    cat ("\n The system diverges! Max number of iteration reached.")
    break
  }
}
