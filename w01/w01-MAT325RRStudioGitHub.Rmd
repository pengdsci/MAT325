...
* **R** is a language and environment for statistical and scientific computing and graphics. It is a GNU project which is similar to the S language and environment which was developed at Bell Laboratories (formerly AT&T, now Lucent Technologies) by John Chambers and colleagues. R can be considered as a different implementation of S. There are some important differences, but much code written for S runs unaltered under R.
...
include_graphics("01-rstudio.png")
```
...
The Environment window shows the objects (i.e., data frames, arrays, values, and functions) in the environment (workspace). We can see descriptive information such as types as the dimension of the objects in your environment. We also choose a data source from the environment to view in the source window like a spreadsheet. 
...
**RPubs** is a free web server provided by RStudio (recently changed to **Posit**) that you can use to publish your analytic reports and code and share with your peers and friends worldwide.

To use this resource, you need to sign up for an account with RPubs first. Once you set up your RPubs account, you can then create reports via RMarkdown and publish them on RPubs in the HTML format. You can share your work with people by providing a hyperlink to them.
...
To use it, you need to create an account. After you set up your GitHub account, you can upload your files (text, code, photos, videos, etc) to the repository. You can also use GitHub to host your web page (static). 

In this class, all data sets you are going to use in your assignments and project are required to be uploaded to your specific repository so you can read your data sets directly from the GitHub repository. 
