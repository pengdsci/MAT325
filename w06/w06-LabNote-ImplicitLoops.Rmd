...
We have discussed the Lagrange form of the interpolation polynomial. In this note, we introduce Newton form of interpolation polynomial.

\

#  Some Definitions

We introduce several definitions related to the divided differences and how to calculated the dived difference manually and programmatically.
...
$$
  ...
  + It is also used to approximated the derivative of the function over interval $[x_i, x_j]$ (assuming $x_i < x_j$) if it exists.
...
The Newton interpolation is defined based on the divided difference. We first look at the structure of the divided difference and based on the pattern to establish an algorithm to compute the divided difference programmatically.
...
We re-organize the above calculation in the following matrix and use the logic to develop the pseudo-code for calculate the divided differences.
...
STEP 2: FOR i from 1 to n DO:
          cumProd = 1             (initial value for cumlative product)
...
NewtonInterpolation = function( vec.x,            # input interpolation nodes
  ...
  Nn                                  # return the value the Newton polynomial
}
...
```
The results are same as those obtained in the textbook of Burden and Faires (9th edition, page 131).
...
Using generalized Rolle theorem repeatedly on the expression of the divided difference, have the following result.
...
$$
Using the theorem introduced in last unit and the uniqueness of the app, we have
...
NewtonInterp = function(xvec, 
 ...
 n=length(xvec)
 xvec0=xvec
 FX = rep(0,n)  # zero vector to store the coefficients
 XX = FX        # zero vector to store the 
 FX[1]=yvec[1]  # The value of f(x0)
 XX[1]=1
 XX0=1
 for (i in 2:n){
    XX[i] = XX0*(pred.x-xvec0[i-1])
    dx = xvec0[i] -xvec0[1]
    dfx = diff(xvec)
    dy = diff(yvec)
    FX[i]=dy[1]/dx
 ...
 Px=sum(FX*XX)
 list(Px = Px, Coefficient = FX, XX = XX)
}
...
```{r}
yvec=c(0.7651977,.6200860,.4554022,.2818186,.1103623)
xvec=c(1,1.3,1.6,1.9,2.2)
NewtonInterp(yvec = yvec, xvec = xvec, pred.x = 1.3)
```
